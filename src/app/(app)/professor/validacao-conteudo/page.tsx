...
export default function ContentValidationPage() {
  ...
  return <div className="view">
    <div className="eyebrow">Revisão pedagógica</div><h1>Validação de conteúdo</h1><p className="muted">Confira as lições de cada nível CEFR e registre a validação com uma observação opcional.</p>
    ...
    <div style={{ display: "grid", gap: 14, marginTop: 20 }}>
      {LEVEL_ORDER.map((level) => {
        ...
        return <div className="card stat" key={level} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(240px,100%),1fr))", gap: 16, alignItems: "end" }}>
          ...
          <button className="btn primary" disabled={busy !== null} onClick={() => void save(level)}>{busy === level ? "Salvando…" : "Salvar"}</button>
        </div>;
      })}
    </div>
  </div>;
