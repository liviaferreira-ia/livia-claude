...
/**
 * Nunca confiar no `file.type` que o navegador manda — é independente da
 * extensão e pode ser forjado (ex.: upload de HTML disfarçado de .pdf, que
 * o link assinado depois serviria e o navegador renderizaria). O tipo real
 * vem sempre da extensão já validada contra a lista permitida.
 */
export function safeContentType(extension: string): string {
  return SAFE_CONTENT_TYPES[extension.toLowerCase()] ?? "application/octet-stream";
}
